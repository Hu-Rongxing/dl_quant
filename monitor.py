        ...
        def wrapper(*args, **kwargs):  
            ...
            while attempts < max_attempts:  
                try:  
                    ...
                    attempts += 1  
                    logger.error(f"函数 {func.__name__} 执行失败。第 {attempts} 次尝试，共 {max_attempts} 次。错误信息：{e}")  
...
if __name__ == '__main__':  
    ...
    scheduler.add_job(download_history_data_task, 'cron', day_of_week='mon-fri', hour='13,16', minute='0', id='download_history_data_task_13_16')  

    # 2. 每个交易日9:00先执行 download_history_data_task，再执行 fit_model_task。  
    # 先在9:00执行download_history_data_task  
    scheduler.add_job(download_history_data_task, 'cron', day_of_week='mon-fri', hour='9', minute='0', id='download_history_data_task_9')  
    ...
    scheduler.add_job(download_history_data_task, 'cron', day_of_week='mon-fri', hour='14', minute='57', id='download_history_data_task_14_57')  
    # 然后在14:58执行buying_strategy_task，确保数据下载完成  
    scheduler.add_job(buying_strategy_task, 'cron', day_of_week='mon-fri', hour='14', minute='58', id='buying_strategy_task')  
    ...
    for rt in report_times:  
        ...
        scheduler.add_job(generate_trading_report_task, 'cron', day_of_week='mon-fri', hour=hour, minute=minute, id=f'generate_trading_report_{rt}')  
    ...
    scheduler.add_job(stop_loss_main_task, 'cron', day_of_week='mon-fri', hour='9-11', minute='30-59/10', id='stop_loss_main_task_morning')  
    scheduler.add_job(stop_loss_main_task, 'cron', day_of_week='mon-fri', hour='13-14', minute='0-59/10', id='stop_loss_main_task_afternoon')  

    # 6. 每个交易日8:30-16:00，每10分钟执行一次 start_xt_client。  
    scheduler.add_job(start_xt_client_task, 'cron', day_of_week='mon-fri', hour='8-15', minute='*/10', id='start_xt_client_task')  
